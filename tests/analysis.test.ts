...
describe("decision analysis", () => {
  it("reproduces the same ensemble and respects the budget in every scenario", () => {
    const first = simulate(defaults),
      second = simulate(defaults);
    ...
    expect(maskCost(first.nominalMask)).toBeLessThanOrEqual(
      defaults.budget + 1e-8,
    );
    expect(maskCost(first.robustMask)).toBeLessThanOrEqual(
      defaults.budget + 1e-8,
    );
    for (const mask of first.optimalMasks)
      expect(maskCost(mask)).toBeLessThanOrEqual(defaults.budget + 1e-8);
  });
  ...
  it("gives a reproducible decision boundary in the sample", () => {
    ...
    expect(portfolioMetrics(run, run.nominalMask).stability).toBeGreaterThan(
      0.1,
    );
    expect(portfolioMetrics(run, run.nominalMask).stability).toBeLessThan(0.8);
    expect(run.factorSwitch.ecology).toBeGreaterThan(0);
    expect(run.robustMask).not.toBe(run.nominalMask);
    expect(maskProjects(run.robustMask).map((p) => p.id)).not.toEqual(
      maskProjects(run.nominalMask).map((p) => p.id),
    );
  });
  ...
  it("reports nonnegative regret against each scenario optimum", () => {
    ...
    for (let i = 0; i < iterations; i++) {
      expect(
        maskValue(run.optimalMasks[i], run.scenarioScores[i]) -
          maskValue(run.nominalMask, run.scenarioScores[i]),
...
describe("unit CSV import", () => {
  it("round trips the sample template including numeric values", () => {
    ...
    expect(simulate(defaults, parsed).nominalMask).toBe(
      simulate(defaults).nominalMask,
    );
  });

  it("rejects malformed values and duplicate ids with a useful error", () => {
    expect(() =>
      parseProjectCsv(templateCsv().replace('"0.91"', '"2"')),
    ).toThrow(/hazard must be between 0 and 1/);
    expect(() =>
      parseProjectCsv(
        templateCsv().replace('"B","Cedar Gap"', '"A","Cedar Gap"'),
      ),
    ).toThrow(/Duplicate unit ID: A/);
  });

  it("reads quoted labels with commas without shifting numeric columns", () => {
    const csv = templateCsv().replace(
      '"North Ridge"',
      '"North Ridge, phase 1"',
    );
    const parsed = parseProjectCsv(csv);
    expect(parsed[0].name).toBe("North Ridge, phase 1");
    expect(parsed[0].hazard).toBe(projects[0].hazard);
  });
});
